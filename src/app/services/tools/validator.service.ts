...
export class ValidatorService {
  ...
  public curp(input: Nullable<unknown>): boolean {
    const s = this.toStr(input).toUpperCase().trim();
    if (s.length !== 18) return false;

    // Expresión regular estándar corregida
    return /^[A-Z]{4}[0-9]{6}[HM][A-Z]{5}[A-Z0-9][0-9]$/.test(s);
  }
  public rfc(input: Nullable<unknown>): boolean {
    ...
    return rfcFisica.test(clean) || rfcMoral.test(clean);
}
